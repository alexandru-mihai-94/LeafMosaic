"""run_demo_svd.py
Pipeline: tile → filter → SVD features → UMAP → cluster → visualize → classify.
This version uses SVD features instead of CNN features.
"""
...
from feature_extraction import features_from_tiles_svd, load_backbone
...
def main():
    parser = argparse.ArgumentParser(description="LeafMosaic demo pipeline with SVD features")
    parser.add_argument("--image", required=True, help="Input image path")
    parser.add_argument("--model", required=True, help="Pretrained Keras model")
    parser.add_argument("--outdir", default="data_svd", help="Output root directory")
    parser.add_argument("--n_components", type=int, default=100,
                        help="Number of SVD components per channel (default: 100)")
    args = parser.parse_args()

    outdir = Path(args.outdir)
    tiles_dir = outdir / "tiles"
    feats_path = outdir / "features" / "features_svd.pkl"
    ...
    # 3. Feature extraction using SVD
    print(f"Extracting SVD features with n_components={args.n_components}...")
    feats = features_from_tiles_svd(tiles, n_components=args.n_components, save_path=feats_path)
